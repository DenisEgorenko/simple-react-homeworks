...
const Greeting: React.FC<GreetingPropsType> = (
    {name, setNameCallback, addUser, error, totalUsers, setErrorToNone, onEnter}) => {

    const inputClass = error ? s.error + ' ' + s.input : s.input // need to fix with (?:)

    return (
        <div className={s.inputBlock}>
            <input onKeyDown={onEnter} onFocus={setErrorToNone} value={name} onChange={setNameCallback} className={inputClass}/>
